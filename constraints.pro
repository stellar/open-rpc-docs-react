...
repo_name(RepoUrl, RepoName) :-
  Prefix = 'https://github.com/stellar/',
...
gen_enforced_field(WorkspaceCwd, 'homepage', CorrectHomepageUrl) :-
  ...
  repo_name(RepoUrl, RepoName),
  atomic_list_concat(['https://github.com/stellar/', RepoName, '#readme'], CorrectHomepageUrl).
...
gen_enforced_field(WorkspaceCwd, 'bugs.url', CorrectBugsUrl) :-
  ...
  workspace_field(WorkspaceCwd, 'repository.url', RepoUrl),
  repo_name(RepoUrl, RepoName),
  atomic_list_concat(['https://github.com/stellar/', RepoName, '/issues'], CorrectBugsUrl).

% The package must specify Git as the repository type.
gen_enforced_field(WorkspaceCwd, 'repository.type', 'git').

% The package must match the URL of a repo within the Stellar organization.
gen_enforced_field(WorkspaceCwd, 'repository.url', 'https://github.com/stellar/<insert repo name here>.git') :-
  workspace_field(WorkspaceCwd, 'repository.url', RepoUrl),
  \+ repo_name(RepoUrl, _).
...
% The package must specify a minimum Node version of 20.
